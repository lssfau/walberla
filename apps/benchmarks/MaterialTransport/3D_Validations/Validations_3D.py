...
# Fluid PDFs and fields
    ...
    #method_concentration.set_conserved_moments_relaxation_rate(1.38)

    pdfs_fluid_setter = pdf_initialization_assignments(
        method_fluid, density=init_density_fluid, velocity=velocity_field.center_vector, pdfs=pdfs_fluid.center_vector
    )

    pdfs_concentration_setter = pdf_initialization_assignments(
    ...
    #update_rule = create_lb_update_rule(lbm_config=lbm_concentration_config, lbm_optimisation=lbm_concentration_opt)
    #update_rule = ps.simp.add_subexpressions_for_field_reads(update_rule)
    generate_sweep(
        ctx,
    ...
    const char * infoCollisionSetup = "{lbm_concentration_config.method}";

...
int main(int argc, char** argv) {
   ...
   // Read config file
   Config::BlockHandle numericalSetup = cfgFile->getBlock("NumericalSetup_old");
   ...
   BlockDataID densityConcentrationFieldID;
      ...
      BlockDataID analyticalConcentrationFieldID = field::addToStorage< DensityField_concentration_T >(blocks, "density concentration field analytical", real_t(0), field::fzyx);
      BlockDataID ErrorFieldID = field::addToStorage< DensityField_concentration_T >(blocks, "density concentration field error", real_t(0), field::fzyx);
...
// Setup of the concentration LBM communication for synchronizing the concentration pdf field between neighboring blocks
   ...
   if (vtkSpacing != uint_t(0))
   {
      ...
      vtkOutput_Concentration->addCellDataWriter(make_shared< field::VTKWriter< DensityField_concentration_T > >(analyticalConcentrationFieldID, "Analytical Concentration"));
      vtkOutput_Concentration->addCellDataWriter(make_shared< field::VTKWriter< DensityField_concentration_T > >(ErrorFieldID, "Error Concentration"));
   ...
   auto calculate_macroscopic_quantities = [&](){for (auto& block : *blocks)
      {
         getterSweep_concentration(&block);
   ...
   analyticalSolGaussian(blocks,analyticalConcentrationFieldID,simulationDomain,domainSize,sigma_0,diffusivity,Uinitialize,x_0,0);
   for (uint_t timeStep = 0; timeStep < timeSteps; ++timeStep)
   {
      ...
      analyticalSolGaussian(blocks,analyticalConcentrationFieldID,simulationDomain,domainSize,sigma_0,diffusivity,Uinitialize,x_0,timeStep);
      std::vector<real_t> norms = computeErrorL2(blocks,densityConcentrationFieldCPUGPUID,analyticalConcentrationFieldID,ErrorFieldID,simulationDomain);

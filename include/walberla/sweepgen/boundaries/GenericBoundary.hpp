...
#include "walberla/v8/sweep/SparseIndexList.hpp"
...
class GenericBoundaryFactory
{
 ...
 public:
   ...
   auto fromFlagField(BlockDataID flagFieldID, field::FlagUID boundaryFlagUID, field::FlagUID fluidFlagUID)
   {
      ...
      using IndexList_T = walberla::v8::sweep::SparseIndexList<
      ...
      for (auto& block : *blocks_)
      {
         ...
         for (auto it = flagField->beginXYZ(); it != flagField->end(); ++it)
         {
            ...
            for (auto dIt = Stencil_T::beginNoCenter(); dIt != Stencil_T::end(); ++dIt)
            {
               ...
               const Cell neighbor{ fluidCell + dir };
               auto dirIdx = cell_idx_c(Stencil_T::idx[dir]);
               if (flagField->isFlagSet(neighbor, boundaryFlag)) { idxVector.emplace_back(fluidCell.x(), fluidCell.y(), fluidCell.z(), dirIdx); }
            }
         }
      }
   ...
   auto fromLinks(TPredicate pred)
   {
      ...
      IndexList_T indexList{ *blocks_ };
      walberla::v8::sweep::SerialSweeper sweeper{ blocks_ };
         ...
         sweeper.forAllCells([&](Cell fluidCell) {
            for (auto dIt = Stencil_T::beginNoCenter(); dIt != Stencil_T::end(); ++dIt)
            {
               ...
               if constexpr ( std::same_as< typename Impl::DataStruct, void > ) {
                  if (pred(link)) {
                     idxVector.emplace_back(fluidCell.x(), fluidCell.y(), fluidCell.z(), dirIdx);
                  }
               } else {
                  auto result = pred(link);
                  if(result.has_value()) {
                     idxVector.emplace_back(fluidCell.x(), fluidCell.y(), fluidCell.z(), dirIdx, *result);
                  }
               }
            }
         });
         ...
         sweeper.forAllCells([&](Cell fluidCell) {
            ...
            if (!walberla::geometry::contains(body, fluidCellCenter)) {
               for (auto dIt = Stencil_T::beginNoCenter(); dIt != Stencil_T::end(); ++dIt)
               {
                  ...
                  if (walberla::geometry::contains(body, wallCellCenter)) {
                     PotentialBoundaryLink link { .block=block, .fluidCell=fluidCell, .dir=dir, .wallCell=wallCell };
                     idxVector.emplace_back(fluidCell.x(), fluidCell.y(), fluidCell.z(), dirIdx, data(link));
                  }
               }
            }
         });

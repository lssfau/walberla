...
generateRayInfo( const Vector3<real_t> & min, const Vector3<real_t> & max, std::vector< real_t > & offsets) 
{
   ...
   for (const auto& offset : offsets) {
      for (const auto& x : xValues) {
         auto x_dir = static_cast< real_t >(floatIsEqual(x , max[0]) ? -1 : floatIsEqual(x , min[0]) ? 1 : 0);
      ...
      for (const auto& y : yValues) {
         auto y_dir = static_cast< real_t >(floatIsEqual(y , max[1]) ? -1 : floatIsEqual(y , min[1]) ? 1 : 0);
      ...
      for (const auto& z : zValues) {
         auto z_dir = static_cast< real_t >(floatIsEqual(z , max[2]) ? -1 : floatIsEqual(z , min[2]) ? 1 : 0);
      ...
      for (const auto& z : zValues) {
         for (const auto& y : yValues) {
            for (const auto& x : xValues) {
               ...
               typename MeshType::Point direction = { x_dir * static_cast< real_t >(floatIsEqual(origin[0] , 0 ) ? 0 : 1),
                                                      y_dir * static_cast< real_t >(floatIsEqual(origin[1] , 0 ) ? 0 : 1),
                                                      z_dir * static_cast< real_t >(floatIsEqual(origin[2] , 0 ) ? 0 : 1)};

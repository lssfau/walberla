...
struct ParticleAndVolumeFractionSoA_T
{
   ...
   ParticleAndVolumeFractionSoA_T(const shared_ptr< StructuredBlockStorage >& bs, const real_t omega)
   {
      ...
      BsFieldID =
         field::addToStorage< BsField_T >(bs, "Bs field CPU", MaxParticlesPerCell, field::fzyx, uint_t(1), true);
      idxFieldID =
         field::addToStorage< idxField_T >(bs, "idx field CPU", MaxParticlesPerCell, field::fzyx, uint_t(1), true);
      BFieldID                  = field::addToStorage< BField_T >(bs, "B field CPU", 1, field::fzyx, uint_t(1), true);
      particleVelocitiesFieldID = field::addToStorage< particleVelocitiesField_T >(
         bs, "particle velocities field CPU", uint_t(MaxParticlesPerCell * 3), field::fzyx, uint_t(1), true);
      particleForcesFieldID = field::addToStorage< particleForcesField_T >(
         bs, "particle forces field CPU", uint_t(MaxParticlesPerCell * 3), field::fzyx, uint_t(1), true);
#endif
      omega_ = omega;
   }
